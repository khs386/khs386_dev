...
| 경로 | 메뉴 | 하는 일 |
|---|---|---|
| `/` | 데일리 브리프 | 모닝브리프, 오늘 기록한 업무, 이번 주 현황(금주 예정), 마감이 가까운 업무(D-7 이내) |
...
셔터스톡·Notion처럼 달마다 빠짐없이 나가야 하는 지출을 `항목 관리 → 반복 결제`에
적어 둡니다. 그 달에 **사용처가 같은 내역이 하나도 없으면** 화면 위에 알리고,
`이 달에 넣기`를 누르면 등록해 둔 값으로 한 줄이 만들어집니다.

- 들어왔는지는 **사용처(가맹점)**로 가립니다. 세부 내역은 달마다 글이 달라지지만
  ("6월 요금" → "7월 요금") 가맹점은 그대로이기 때문입니다.
- 기간(`시작 달 ~ 끝 달`)을 비우면 끝이 없다는 뜻입니다. 셔터스톡처럼 몇 달만
  구독하는 것은 기간을 넣어 두면 그 밖의 달에는 알리지 않습니다.
- `씀` 체크를 풀면 지우지 않고 잠시 꺼 둘 수 있습니다.
- 자동결제는 영수증이 눈에 띄지 않아 조용히 빠집니다. 노션에서 옮겨 온 21건에서도
  셔터스톡·Notion이 6월 뒤로 끊겨 있었습니다.
