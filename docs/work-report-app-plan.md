...
| 항목 | 결정 |
|---|---|
| 데이터 입력 | 앱에서 직접 입력. 노션은 더 읽지 않는다 |
| 시리즈 총 진행률 | `/series` 화면에서 숫자를 직접 입력 |
| Supabase 프로젝트 | morning-brief와 **분리**된 새 프로젝트 |
| 최종 전달 | 구글 드라이브 지정 폴더에 저장 (서비스 계정, 같은 이름이면 덮어쓰기) |

이 결정에 따라 7절의 노션 이관 스크립트는 만들지 않았다. 기존 데이터는
`/tasks`와 `/weekly` 화면에서 직접 옮긴다.

## 10. 구현 결과

1~2단계와 4단계를 `work-report/`에 구현했다. 자세한 설정 방법은
`work-report/README.md`에 있다.

- **렌더러**: `lib/report/` — 색상, 날짜, 일일·주간 HTML 조립
- **데이터 계층**: `lib/reportData.js` — DB 행을 렌더러 입력으로 바꾸고 이력에 저장
- **드라이브**: `lib/drive.js` — 서비스 계정 JWT를 직접 서명해 업로드. 추가 패키지 없음
- **자동 생성**: `app/api/cron/` + `vercel.json` — 평일 18시, 금요일 17시 (KST)
- **화면**: `/`, `/tasks`, `/daily`, `/weekly`, `/series`, `/reports`
- **검증**: `npm test` 16건. 첨부 결과물 2건과 바이트 단위로 일치하는지 확인한다

남은 일은 실제 Supabase 프로젝트와 구글 서비스 계정을 만들어 환경변수를 채우고,
Vercel에 배포한 뒤 실제 데이터로 한 번 돌려 보는 것이다.
