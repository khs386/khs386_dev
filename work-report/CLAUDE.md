...
- 배포는 이 폴더에서 `npx wrangler deploy`로 합니다. 사용자가 직접 하기도 하고,
  시키면 Claude가 하기도 합니다. **다만 스스로 판단해 배포하지는 마세요** — 코드를
  고쳤으면 배포가 필요하다고 알리고, 하라고 할 때 합니다.
- DB를 바꿨으면 **마이그레이션을 먼저 돌리고 배포합니다.** 순서가 바뀌면 새 화면이
  없는 표를 찾다가 깨집니다. 돌린 뒤에는 원격에서 실제로 세어 확인하세요.
...
2. **모든 변경 뒤 `npm test`.** 하나도 빠짐없이 통과해야 합니다 (지금 58개).
...
## 어디에 무엇이 있는가
    ...
    src/lib/series.js     시리즈 총 진행률 — 단계와 몫은 시리즈마다 DB에 있다
    src/lib/tasks.js      마감이 가까운 업무 고르기
    src/lib/voucher.js    지출결의서 서식 — report/ 와 달리 골든 테스트에 묶여 있지 않다
